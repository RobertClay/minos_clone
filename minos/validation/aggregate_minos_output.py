...
def aggregate_variables_by_year(source, years, tag, v, method, subset_func):
    ...
    for year in years:
        ...
        if year == 2018:
            subset_func_2018 = find_subset_function('who_alive')
            with Pool() as pool:
                aggregated_means = pool.starmap(aggregate_csv,
                                                zip(files, repeat(v), repeat(method), repeat(subset_func_2018)))
        else:
            with Pool() as pool:
                aggregated_means = pool.starmap(aggregate_csv,
...
if __name__ == '__main__':
    ...
    for directory, tag, subset_function_string in zip(directories, tags, subset_functions):
        ...
        if len(runtime) > 1:
            ...
            runtime = str(runtime[0])  # os.listdir returns a list, we only have 1 element
        else:
